...
class PengeluaranController {
    ...
    async getPengeluaran(req, res) {
        try {
            ...
            const result = await pengeluaranService.getPengeluaran(userId);
            ...
            console.error(error);
            sendResponse(500, req.body, "Error retrieving pengeluaran: " + error.message, res);
        }
    }
    ...
    async addPengeluaran(req, res) {
        try {
            const userId = await auth.getUserAuthenticate(req.user);
            const result = await pengeluaranService.addPengeluaran({ ...req.body, isPengeluaran: true }, userId);

            sendResponse(200, result, "Data successfully added", res, true);
        } catch (error) {
            sendResponse(500, req.body, "Error adding pengeluaran service: " + error.message, res);
        }
    }
    ...
    async updatePengeluaran(req, res) {
        try {
            const { id } = req.params;
            const result = await pengeluaranService.updatePengeluaran(id, req.body, req.user);
    ...
    async addPengeluaranByGPT(req, res) {
        try {
            const result = await pengeluaranService.addPengeluaranByGPT(req.body, req.user);
            sendResponse(200, result, "Data successfully added", res, true);
        } catch (error) {
            sendResponse(500, req.body, "Error adding pengeluaran: " + error.message, res);
        }
    }
}
